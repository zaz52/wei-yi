...
```bash
cd analytics-worker
wrangler d1 create wei-yi-analytics-db
```

执行后会输出类似：

```
✅ Successfully created DB 'wei-yi-analytics-db'
...
```bash
wrangler d1 execute wei-yi-analytics-db --remote --file=./schema.sql
```
...
```
https://wei-yi-analytics.你的子域名.workers.dev
```
...
```typescript
const ANALYTICS_URL = "https://wei-yi-analytics.你的子域名.workers.dev"
```
...
直接访问：
```
https://wei-yi-analytics.你的子域名.workers.dev/dashboard?key=你设置的密钥
```
...
curl -H "Authorization: Bearer 你设置的密钥" \
  https://wei-yi-analytics.你的子域名.workers.dev/stats
```
