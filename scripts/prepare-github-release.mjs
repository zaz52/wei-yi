...
const portableExe = resolve(root, "release-portable/WeiYi.exe")
...
const releaseAssetName = `WeiYi_${pkg.version}_windows_X64${extname(updaterAssetName)}`
const releaseAssetPath = resolve(outDir, releaseAssetName)
const releaseSignaturePath = `${releaseAssetPath}.sig`
const releaseBaseUrl = process.env.WEI_YI_RELEASE_BASE_URL

if (!releaseBaseUrl) {
  throw new Error("Missing WEI_YI_RELEASE_BASE_URL, for example: https://github.com/owner/repo/releases/latest/download")
}
...
const privateKeyPath = process.env.TAURI_SIGNING_PRIVATE_KEY_PATH || resolve(process.env.USERPROFILE ?? "", ".tauri/wei-yi-updater.key")
...
if (existsSync(portableExe)) {
  cpSync(portableExe, resolve(outDir, "WeiYi-portable.exe"))
}

const signature = readFileSync(releaseSignaturePath, "utf8").trim()
const latest = {
  version: pkg.version,
  notes: `唯一 ${pkg.version} release`,
